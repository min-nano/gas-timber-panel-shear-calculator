...
**1 スプレッドシート = 1 物件（プロジェクト）** とし、1 つの物件が複数の「パターン」
（例: 同一物件内の複数の面材配置）を持てます。アプリ上ではパターンをページ送りで
切り替えて編集できます（ページネーション）。保存は **2 タブ構成**です。

| タブ | 役割 | 行の意味 | 更新方法 |
|------|------|---------|---------|
| `パターン`（現在値） | 参照される正本 | 1 行 = 1 パターン | `patternId` で **upsert**（同じパターンは同じ行を上書き） |
| `履歴` | 構造化した時系列ログ | 1 行 = 1 保存 | 常に **追記** |

入力値だけでなく **計算結果（Ixy・Zxy・Cxy ほか）も同じ行に列として記録**するため、
別のシート・帳票・スクリプトから参照するとき、再計算せずに結果を直接使えます。
`パターン` タブは `patternId` で行が固定される（upsert）ので、**「列＝フィールド」「行＝パターン」
の 2 次元でセル参照が安定**します。

> **Google ドライブのファイル版管理**は、ファイル丸ごと・自動間引き・クエリ不可のため、
> 誤操作からの復旧という**保険**にとどめ、構造化した履歴は上記 `履歴` タブに持たせています。
...
バージョン番号（`SHEET_SCHEMA_VERSION`）で管理**しています（現在 **v2**）。
...
- 列の追加・削除・並べ替え・意味変更を行ったら `SHEET_SCHEMA_VERSION` を上げます
  （過去行の版値は変更しません）。v2 では物件名・パターン ID・パターン名（識別列）を追加しました。
...
| 関数 | 役割 |
|------|------|
| `saveCalculationApi(payload)` | 計算し直した結果ごと保存。`パターン` タブへ upsert＋`履歴` タブへ追記。初回は新規スプレッドシートを作成し ID を返す（以降は同一物件へ集約） |
| `listPatternsApi(payload)` | 物件（スプレッドシート）内の全パターン（`パターン` タブ）を返す。既存物件の読み込み・切替に使う |
| `getSheetSchemaApi()` | セルマッピング スキーマ（版・キー列・タブ名・列一覧・A1 列文字）を返す |
...
- [x] **フェーズ2**: 1 物件・複数パターンをスプレッドシート保存（現在値 upsert＋履歴 追記、入力＋結果）。セルマッピングをスキーマ化しバージョン管理（v2）
